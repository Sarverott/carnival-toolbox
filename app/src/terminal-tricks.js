...
class CliTool {
  ...
  paintIt(...codes) {
    return codes.map((colorcode) => `\x1b[${colorcode}m`).join("");
  }
  rainbowize(
    data,
    mode = "FRONT",
  ...
  static get PTXML_REPLACE_COLORS() {
    ...
    for (var i in colorCodes) {
      if (typeof colorCodes[i] == "number") {
        result[i] = [`\x1b[${colorCodes[i].toString()}m`, "\x1b[0m"];
      }
    }
    for (var i in colorCodes.FRONT) {
      result["f-" + i] = [`\x1b[${colorCodes.FRONT[i].toString()}m`, "\x1b[0m"];
    }
    for (var i in colorCodes.BACK) {
      result["b-" + i] = [`\x1b[${colorCodes.BACK[i].toString()}m`, "\x1b[0m"];
    }
    return result;
  }
  ring(i = 1) {
...
class HeaderTitle extends CliTool {
  ...
  paint(mode, ...opts) {
    switch (mode) {
      case "random":
        this.prerender = this.rainbowize(this.orginal, opts[0], {
          useColors: opts[1],
          banColors: [],
        });
        break;
      case "color":
        this.prerender = this.colorize(this.orginal, opts);
        break;
      case "not":
        this.prerender = this.orginal;
        break;
  ...
  shades(modifyer = this.dim, colorUnify = "") {
    ...
    for (var shadow of dimmingList) {
      tmpEdit = tmpEdit.replaceAll(
        shadow,
        colorUnify + this.colorize(shadow, [modifyer, this.reset])
      );
    }
    this.render = tmpEdit;
    return this;
